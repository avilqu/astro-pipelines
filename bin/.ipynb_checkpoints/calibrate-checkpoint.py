...
def findMasterDark(image, tempTolerance=1, expTolerance=0.5):
    ...
    if (match):
        ...
        print('Could not find a suitable master dark for exposure {} and temperature {}C.'.format(exposure, temp))
...
def findCalibratedMasterDark(image, tempTolerance=1):
    ...
    match = False
    for img, fname in calibrationMasters.ccds(frame='Dark', return_fname=True):
        if (abs(img.header['ccd-temp'] - temp) <= tempTolerance and img.header['exptime'] >= exposure):
            match = img
            masterDark = fname
            break
    if (match):
        ...
        print('Could not find a suitable calibrated master dark for exposure {} and temperature {}C.'.format(exposure, temp))
...
def findMasterFlat(image, tempTolerance=1):
    ...
    if (match):
        ...
        print('Could not find a suitable master flat for filter {} and temperature {}C.'.format(filter, temp))
...
def imageCalibration(img, fname, options):
    ...
    if (options.biasonly):
        ...
        if (masterDark):
            ...
            img = ccdp.subtract_dark(img, masterDark, exposure_time='exptime', exposure_unit=u.second)
            ...
            if (masterDark):
                ...
                if (masterDark):
                    ...
                    img = ccdp.subtract_dark(img, masterDark, exposure_time='exptime', exposure_unit=u.second, scale=True)
            else: print('No dark or bias substraction.')
        ...
        else: print('Skipping flat correction.')
...
def calibrateCollection(collection, options):
    if (options.write):
        ...
        for img, fname in collection.ccds(return_fname=True):
            imageCalibration(img, fname, options).write(calibratedPath / fname, overwrite=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Basic image reduction script. Reads a list of calibration masters from the calibrationPath variable and finds the best match for each frame.')
    parser.add_argument('-d', '--dir', action='store_true', help='calibrate all fits files in current directory')
    parser.add_argument('-f', '--files', nargs="+", help='select fits files to calibrate')
    parser.add_argument('-n', '--noflat', action='store_true', help='skip flat calibration')
    parser.add_argument('-b', '--biasonly', action='store_true', help='only do bias substraction')
    parser.add_argument('-p', '--platesolve', action='store_true', help='platesolve images with local astromery index')
    parser.add_argument('-w', '--write', action='store_true', help='write file (default=True)', default=True)
    ...
    if args.dir:
        lightImages = ccdp.ImageFileCollection(os.getcwd()).filter(frame='light')
    elif args.files:
        lightImages = ccdp.ImageFileCollection(filenames=args.files).filter(frame='light')
    else:
    ...
    if 'lightImages' in locals():
        ...
        print(calibrationMasters.summary['file', 'frame', 'instrume', 'filter' ,'exptime', 'ccd-temp', 'naxis1', 'naxis2'])
        # print(calibrationMasters.summary['file', 'frame', 'filter' ,'exptime', 'ccd-temp', 'naxis1', 'naxis2'])
        print('\nFiles to calibrate:')
        print(lightImages.summary['object', 'date-obs', 'frame', 'instrume', 'filter', 'exptime', 'ccd-temp', 'naxis1', 'naxis2'])
        ...
        for img, fname in lightImages.ccds(return_fname=True):
            imageCalibration(img, fname, args).write(calibratedPath / fname, overwrite=True)
